...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    header("Location: company_manage_courses.php");
    exit();
}
...
// Get course details
$stmt = $conn->prepare("SELECT * FROM courses WHERE course_id = ? AND created_by_type = 'company' AND created_by_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($title) || empty($description) || empty($duration) || empty($mode) || empty($category) || empty($start_date) || empty($end_date)) {
        ...
        // Update course
        $sql = "UPDATE courses SET course_title = ?, course_description = ?, course_duration = ?, is_online = ?, course_category = ?, start_date = ?, end_date = ?, instructor_id = ?, location = ?, requirements = ?, learning_outcomes = ?, certificate_option = ?, status = ?, updated_at = NOW() 
                WHERE course_id = ? AND created_by_type = 'company' AND created_by_id = ?";
        
        $stmt = $conn->prepare($sql);
        // Parameter types: title(s), description(s), duration(s), is_online(i), category(s), start_date(s), end_date(s), instructor_id(i), location(s), requirements(s), learning_outcomes(s), certificate_option(i), status(s), course_id(i), company_id(i)
        $stmt->bind_param("sssissssssssiii", $title, $description, $duration, $is_online, $category, $start_date, $end_date, $instructor_id, $location, $requirements, $learning_outcomes, $certificate_option, $status, $course_id, $company_id);
...
<head>
    ...
    <title>Edit Course - Company Portal</title>
    ...
    <div class="container">
        <div class="header">
            <div class="breadcrumb">
                ...
                <a href="company_manage_courses.php">Manage Courses</a>
                <i class="fas fa-chevron-right"></i>
                <span>Edit Course</span>
            </div>
            <h1 class="page-title">Edit Course</h1>
            <p class="page-subtitle">Update course information and settings</p>
        </div>
        ...
        <div class="form-container">
            ...
            <form method="POST" action="">
                <?php 
                // Convert database columns to display values
                $course_title = $course['course_title'] ?? '';
                $course_description = $course['course_description'] ?? '';
                $course_duration = $course['course_duration'] ?? '';
                $mode = isset($course['is_online']) && $course['is_online'] == 1 ? 'Online' : 'Onsite';
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" class="form-input form-textarea" 
                              placeholder="Describe the course content, objectives, and what students will learn" required><?php echo htmlspecialchars($course_description); ?></textarea>
                </div>

                <div class="form-group">
                    <label for="duration" class="form-label required">Duration</label>
                    <input type="text" id="duration" name="duration" class="form-input" 
                           value="<?php echo htmlspecialchars($course_duration); ?>" 
                ...
                <div class="form-group">
                    ...
                    <select id="category" name="category" class="form-select" required>
                        ...
                        <option value="soft_skills" <?php echo ($category === 'soft_skills') ? 'selected' : ''; ?>>Soft Skills</option>
                        <option value="certification" <?php echo ($category === 'certification') ? 'selected' : ''; ?>>Certification</option>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="location" name="location" class="form-input" 
                           value="<?php echo htmlspecialchars($course['location'] ?? ''); ?>" 
                ...
                <div class="form-group">
                    ...
                    <textarea id="requirements" name="requirements" class="form-input form-textarea" 
                              placeholder="List any prerequisites, skills, or requirements for this course"><?php echo htmlspecialchars($course['requirements'] ?? ''); ?></textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="learning_outcomes" name="learning_outcomes" class="form-input form-textarea" 
                              placeholder="Describe what students will learn and achieve after completing this course"><?php echo htmlspecialchars($course['learning_outcomes'] ?? ''); ?></textarea>
                </div>

                <div class="form-group">
                    <label class="form-label">
                        <input type="checkbox" id="certificate_option" name="certificate_option" value="1" 
                               <?php echo ($course['certificate_option'] ?? 0) ? 'checked' : ''; ?>>
                        Certificate Option
                    </label>
                ...
                <div class="form-actions">
                    <a href="company_manage_courses.php" class="btn btn-secondary">
