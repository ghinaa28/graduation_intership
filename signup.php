...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(empty($error)) {
        ...
        if(isset($_POST['first_name']) && isset($_POST['last_name']) && isset($_POST['university']) && !isset($_POST['company_name']) && !isset($_POST['bio'])) {
            ...
            $training_interests = !empty($_POST['training_interests']) ? $conn->real_escape_string($_POST['training_interests']) : null;
            $preferred_fields = !empty($_POST['preferred_fields']) ? $conn->real_escape_string($_POST['preferred_fields']) : null;
            $skills = !empty($_POST['skills']) ? $conn->real_escape_string($_POST['skills']) : null;
            $profile_photo = uploadFile($_FILES['profile_photo'], "uploads/students/profile_photos", ['jpg','jpeg','png']);
            $cv_path = uploadFile($_FILES['cv'], "uploads/students/cv", ['pdf']);
            $instructor_id = !empty($_POST['instructor_id']) ? intval($_POST['instructor_id']) : null;
            $status = 'pending'; // Default status for new students

            $sql = "INSERT INTO students (first_name, last_name, university, email, password, department, phone, student_id_number, academic_year, gpa, expected_graduation, date_of_birth, training_interests, preferred_fields, skills, profile_photo, cv_path, instructor_id, status, created_at)
                    VALUES ('$first_name', '$last_name', '$university', '$email', '$password', '$department', " . ($phone ? "'$phone'" : "NULL") . ", '$student_id_number', '$academic_year', " . ($gpa ? "'$gpa'" : "NULL") . ", " . ($expected_graduation ? "'$expected_graduation'" : "NULL") . ", " . ($date_of_birth ? "'$date_of_birth'" : "NULL") . ", " . ($training_interests ? "'$training_interests'" : "NULL") . ", " . ($preferred_fields ? "'$preferred_fields'" : "NULL") . ", " . ($skills ? "'$skills'" : "NULL") . ", " . ($profile_photo ? "'$profile_photo'" : "NULL") . ", " . ($cv_path ? "'$cv_path'" : "NULL") . ", " . ($instructor_id ? "$instructor_id" : "NULL") . ", '$status', NOW())";
...
.form-group input[type="month"] + .input-icon {
  pointer-events: none;
}
      ...
      <div class="role-selection" id="roleSelection">
          ...
          <form class="signup-form" id="studentForm" action="signup.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <input type="date" name="date_of_birth" class="form-input" placeholder="Select your date of birth">
              <i class="fas fa-birthday-cake input-icon"></i>
            </div>

            <div class="form-group">
              <label class="form-label">Training Interests (Optional)</label>
              <textarea name="training_interests" class="form-textarea" placeholder="Describe your training interests and what you hope to learn" rows="3"></textarea>
            </div>

            <div class="form-group">
              <label class="form-label">Preferred Fields (Optional)</label>
              <textarea name="preferred_fields" class="form-textarea" placeholder="List your preferred fields of work or study (e.g., Software Development, Marketing, Finance)" rows="3"></textarea>
            </div>

            <div class="form-group">
              <label class="form-label">Skills (Optional)</label>
              <textarea name="skills" class="form-textarea" placeholder="List your skills (e.g., Python, JavaScript, Project Management, Communication)" rows="3"></textarea>
            </div>
