...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    switch ($action) {
        case 'delete':
            $stmt = $conn->prepare("DELETE FROM courses WHERE course_id = ? AND created_by_type = 'company' AND created_by_id = ?");
        ...
        case 'deactivate':
            $stmt = $conn->prepare("UPDATE courses SET status = 'inactive' WHERE course_id = ? AND created_by_type = 'company' AND created_by_id = ?");
            $stmt->bind_param("ii", $course_id, $company_id);
            if ($stmt->execute()) {
        ...
        case 'activate':
            $stmt = $conn->prepare("UPDATE courses SET status = 'active' WHERE course_id = ? AND created_by_type = 'company' AND created_by_id = ?");
            $stmt->bind_param("ii", $course_id, $company_id);
            if ($stmt->execute()) {
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php foreach ($courses as $course): 
                            // Convert is_online (0/1) to mode string (Onsite/Online)
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <span class="mode-badge"><?php echo htmlspecialchars($mode); ?></span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <?php 
                                        // Handle both 'id' and 'course_id' column names
                                        $course_id_value = $course['course_id'] ?? $course['id'] ?? '';
                                        ?>
                                        <a href="company_edit_course.php?id=<?php echo $course_id_value; ?>" class="btn btn-primary">
                                        ...
                                        <?php if ($course['status'] === 'active'): ?>
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <input type="hidden" name="course_id" value="<?php echo $course_id_value; ?>">
